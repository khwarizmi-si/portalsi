...
grep address /home/app.portalsi.com/public_html/apps/web/deploy-vps/vhost-proxy.conf

# e. Port yang dipakai Node
grep -E '^PORT' /home/app.portalsi.com/public_html/apps/web/.env.production
```
...
cd /home/app.portalsi.com/public_html
chmod +x apps/web/deploy-vps/*.sh
...
2. Tempel **kedua blok** dari `apps/web/deploy-vps/vhost-proxy.conf`
...
```bash
cd /home/app.portalsi.com/public_html
apps/web/deploy-vps/install-post-merge-hook.sh
```
...
- [ ] `build/index.js` ada (build sukses) — cek `apps/web/build/index.js`
...
Solusi proyek ini: skrip `apps/web/scripts/patch-csrf.mjs` menonaktifkan dua cek
...
```bash
cd /home/app.portalsi.com/public_html/apps/web
...
cd /home/app.portalsi.com/public_html
git pull --ff-only
apps/web/deploy-vps/deploy-after-pull.sh   # build kini sudah termasuk patch CSRF
```
